...
![](images/clipboard-31491687.png)

Figure 1 RF (a) and KNN (b) predicted outcome

Have you ever wondered if this same algorithms are **applied** **to** urban area with **different morphology** what will happen? Because we know that each city experiences different processes and mechanisms, resulting in unique urban conditions and morphologies. The answer is yes ! Although they come from different process @ibrahim2019 highlight that informal settlements still have unique identifier that make them distinguishable from formal settlements. However, they also mentioned that accuracy improves when the model is trained in areas close to the prediction site.
